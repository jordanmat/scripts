...
for env_file in $(find . -type f -path "*/kustomization-postbuild-variables/env.yaml"); do
    ...
    # Query the prometheus_tsdb_head_series metric
    response=$(curl -sk -w "%{http_code}" -o /tmp/curl_response.txt "${PROMETHEUS_URL}/api/v1/query?query=prometheus_tsdb_head_series")
    ...
    # Extract the number of active time series
    timeseries_count=$(echo "$raw_response" | jq -r '.data.result[0].value[1]' 2>/dev/null)

    if [[ -z "$timeseries_count" || "$timeseries_count" == "null" ]]; then
        continue
    fi

    # Append the environment name and time series count to the temporary file
    echo -e "${ENV_NAME}\t${timeseries_count}" >> "$tmp_file"
done
